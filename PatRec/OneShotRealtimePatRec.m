...
function [outMov, outVector, patRec, handles] = OneShotRealtimePatRec(tData, patRec, handles, thresholdGUIData, idata)


    if ~isempty(idata)
        % Signal processing
        tSet = SignalProcessing_RealtimePatRec(tData, patRec, idata);
    else
        % Signal processing
        idata = []; % spaceholder
        tSet = SignalProcessing_RealtimePatRec(tData, patRec, idata);
